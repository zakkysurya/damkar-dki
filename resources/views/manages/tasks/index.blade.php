                        ...
                        <table id="data-table" width="100%" cellspacing="0" class="table table-striped table-bordered nowrap w-100">
                              <thead>
                                    <tr>
                                          ...
                                          <th scope="col" class="align-middle">Judul Task</th>
...
<script type="text/javascript">
      ...
      $(function() {
            ajaxDataTable = initDataTable({
                  selector: '#data-table',
                  url: "{!! route('manages.tasks.data-table') !!}",
                  columns: [{
                              data: 'DT_RowIndex',
                              orderable: false,
                              searchable: false,
            ...
            const EventsHandler = {
                  ...
                  html: {
                      ...
                      <div class="modal-body">
                        <div class="row">
                          <div class="col-12 mb-2">
                            <label for="nama_lengkap">Judul Task <span class="text-danger">*</span></label>
                            <input 
                              type="text" 
                              name="judul_task" 
                              class="form-control"
                              placeholder="Masukan Judul Tasks" />
                          </div>
                          <div class="col-12 mb-2">
                            ...
                            <select class="form-control" name="project_id">
													...
													<div class="col-12 mb-2">
                            <label for="man_power_id">Man Powers <span class="text-danger">*</span></label>
														<select class="form-control" name="man_power_id">
															<option disabled selected>Pilih Man Powers: </option>
															@forelse ($man_powers as $mp)
																<option value="{{ $mp->id }}">{{ $mp->nama_lengkap }}</option>
															@empty
																<option>Data Tidak Ditemukan.</option>
															@endforelse
														</select>
                  ...
                  ajax: {
                        ...
                        save: function(formData) {
                              $.ajax({
                                    url: "{!! route('manages.tasks.store') !!}", // Endpoint Laravel
                                    type: 'POST',
                                    data: formData,
                        ...
                        show: function(encryptedId) {
                              $.ajax({
                                    url: "{!! route('manages.tasks.show') !!}", // Endpoint Laravel
                                    type: 'get',
                                    data: {
                                          encryptedId
                                    },
                      ...
                      <div class="modal-body">
                        <div class="row">
                        	<div class="col-12 mb-2">
                            <label for="judul_task">Judul Task <span class="text-danger">*</span></label>
                            <input 
                              type="text" 
                              name="judul_task" 
                              class="form-control"
                              placeholder="Masukan Judul Tasks"
															value="${response.data.judul_task}" />
                          </div>
                          <div class="col-12 mb-2">
                            <label for="project_id">Project <span class="text-danger">*</span></label>
                            <select class="form-control" name="project_id">
															...
															@forelse ($projects as $project)
																<option value="{{ $project->id }}" ${response.data.project_id == "{{ $project->id }}" ? "selected": ""} >{{ $project->nama_project }}</option>
															@empty
																<option>Data Tidak Ditemukan.</option>
															@endforelse
														</select>
                            ...
                            <label for="man_power_id">Man Powers <span class="text-danger">*</span></label>
														<select class="form-control" name="man_power_id">
															<option disabled selected>Pilih Man Powers: </option>
															@forelse ($man_powers as $mp)
																<option value="{{ $mp->id }}" ${response.data.man_power_id == "{{ $mp->id }}" ? "selected": ""} >{{ $mp->nama_lengkap }}</option>
															@empty
																<option>Data Tidak Ditemukan.</option>
															@endforelse
														</select>
                          ...
                          <div class="col-12 mb-2">
                            ...
                            <select class="form-control" name="status">
															...
															<option value="in_progress" ${response.data.status == "in_progress" ? "selected" : ''}>in_progress</option>
															<option value="pending" ${response.data.status == "pending" ? "selected" : ''}>pending</option>
															<option value="done" ${response.data.status == "done" ? "selected" : ''}>done</option>
														</select>
													</div>
                        </div>
                      </div>
                      <div class="modal-footer d-flex justify-content-end">
                        ...
                        update: function(formData) {
                              $.ajax({
                                    url: "{!! route('manages.tasks.update') !!}", // Endpoint Laravel
                                    type: 'POST',
                                    data: formData,
                                    processData: false, // Jangan proses data karena menggunakan FormData
                                    contentType: false, // Jangan set content-type secara otomatis
                        ...
                        destroy: function(encryptedId) {
                              $.ajax({
                                    url: "{!! route('manages.tasks.destroy') !!}", // Pastikan ini di-render dengan benar
                                    type: 'POST',
                                    data: {
